...
public class FPPlayerListener extends PlayerListener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (((event.getAction() == Action.LEFT_CLICK_BLOCK) || (event.getAction() == Action.LEFT_CLICK_AIR))
            ...
            player.getWorld().spawn(player.getEyeLocation().add(direction.getX(),
        ...
        if (((event.getAction() == Action.RIGHT_CLICK_BLOCK) || (event.getAction() == Action.RIGHT_CLICK_AIR))
            ...
            event.getPlayer().getWorld().strikeLightning(
                    event.getPlayer().getTargetBlock(null, 500).getLocation());
        }
        if (((event.getAction() == Action.LEFT_CLICK_BLOCK) || (event.getAction() == Action.LEFT_CLICK_AIR))
                && (event.hasItem())
            ...
            if (state instanceof Sign) {
                ...
                if ((sign.getLines()[0].equalsIgnoreCase("["
                    ...
                    if (!(sign.getLines()[1].isEmpty())) {
                        ...
                        event.getPlayer().sendMessage(FPSettings.incorrectusage);
                    }
                }
            }
        }
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if (FPSettings.Secure) {
            if (utils.getProperty(utils.savedsettings(),
                ...
                String trueip = player.getAddress().getAddress().getHostAddress();
                if (flagged) {
    ...
    public void onPlayerTeleport(PlayerTeleportEvent event) {
        ...
        if (FPSettings.GodOnTeleportOrJoin) {
            if (!GodCommand.tempGods.containsKey(player.getEntityId())) {
                int id = parent.getServer().getScheduler().scheduleSyncDelayedTask(parent,
        ...
        if (WeatherVoteCommand.inProgress.containsKey(player.getWorld())) {
            if (WeatherVoteCommand.Yes.containsKey(player)) {
                ...
                if (Old != New) {
                    WeatherVoteCommand.Yes.remove(player);
                    player.sendMessage(FPSettings.weathervoteinprogress.replace("@t", WeatherVoteCommand.Type.get(player.getWorld())));
                }
            }
            if (WeatherVoteCommand.No.containsKey(player)) {
        ...
        if (FPSettings.BedVoting) {
            if (FPPermissions.has(event.getPlayer(), "featurepack.bed")) {
                if (!TimeVoteCommand.inProgress.containsKey(player.getWorld())) {
                    ...
                    if (TimeVoteCommand.inProgress.get(event.getPlayer().getWorld()) == false) {
                        player.sendMessage(FPSettings.timecooldown);
                    } else {
                        player.sendMessage(FPSettings.timevoteinprogress.replace("@t", TimeVoteCommand.Type.get(player.getWorld())));
                    }
                }
            }
        }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (FPSettings.BiomeMessages) {
            if (!event.getTo().getBlock().getBiome().equals(event.getFrom().getBlock().getBiome())) {
                event.getPlayer().sendMessage(
                        FPSettings.enteredbiome.replace("@b", event.getTo().getBlock().getBiome().name().toLowerCase().replace("_", " ")));
            }
        }
        if (FPSettings.Regen) {
            if (RegenCommand.Regen.containsKey(player)) {
