            ...
            if (args.length == 2) {
                ...
                if (args[1].equalsIgnoreCase("?")) {
                    ...
                    if (Secure.containsKey(player)) {
                        if (args[2].equals(utils.get(utils.savedsettings()).getString(
                                "Secure." + player.getName()
                                + ".pass"))) {
                            utils.setProperty(utils.savedsettings(), "Secure."
                                    + player.getName() + ".ip", player.getAddress().getAddress().getHostAddress());
                            utils.setProperty(utils.savedsettings(), "Secure."
                    ...
                    if (FPPermissions.has(player, "featurepack.secure.override")) {
                        for (Player who : parent.getServer().getOnlinePlayers()) {
                            if (who.getName().contains(args[2])) {
                                utils.setProperty(utils.savedsettings(),
                                        "Secure." + who.getName() + ".ip", who.getAddress().getAddress().getHostAddress());
                                utils.setProperty(utils.savedsettings(),
                    ...
                    if (utils.get(utils.savedsettings()).getString(
                            "Secure." + player.getName()) != null) {
                        if (utils.get(utils.savedsettings()).getString(
                                "Secure." + player.getName() + ".pass").equals(args[2])) {
